...
public class GeneralFuncController : MonoBehaviour
{
    ...
    public float SCROLL_END_LEFT = -15f; // 左側への移動制限(これ以上左には進まない)
    public float SCROLL_END_RIGHT = 15f; // 右側への移動制限(これ以上右には進まない)
    public float SCROLL_DISTANCE_CORRECTION = 0.8f; // スクロール距離の調整
